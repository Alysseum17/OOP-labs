...
// Встановлення координат для фігури
void Shape::Set(LONG ax1, LONG ay1, LONG ax2, LONG ay2) {
    x1 = ax1; y1 = ay1; x2 = ax2; y2 = ay2;
}

// Реалізація методів Show для кожної фігури
...
void RectShape::Show(HDC hdc) const {
    HBRUSH hBrush = CreateSolidBrush(RGB(255, 192, 203)); // Рожевий колір заповнення
...
void EllipseShape::Show(HDC hdc) const {
    HBRUSH hOldBrush = (HBRUSH)SelectObject(hdc, GetStockObject(WHITE_BRUSH)); // Біле заповнення
    HPEN hPen = CreatePen(PS_SOLID, 1, RGB(0, 0, 0));
    HPEN hOldPen = (HPEN)SelectObject(hdc, hPen);
...
// Реалізація для лінії з кружечками
...
void CubeShape::Show(HDC hdc) const {
    ...
    // Малюємо задній прямокутник (викликаючи метод базового класу)
    ...
    // Малюємо з'єднувальні лінії (викликаючи метод базового класу)
