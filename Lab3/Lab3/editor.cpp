// Повний остаточний вміст для editor.cpp
...
void LineEditor::OnPaint(HWND hWnd, HDC hdc) {
    if (x1 != x2 || y1 != y2) {
        HPEN hPen = CreatePen(PS_SOLID, 1, RGB(255, 0, 0)); // Червоний слід
...
void RectEditor::OnPaint(HWND hWnd, HDC hdc) {
    if (x1 != x2 || y1 != y2) {
        HPEN hPen = CreatePen(PS_SOLID, 1, RGB(255, 0, 0)); // Червоний слід
        HPEN hOldPen = (HPEN)SelectObject(hdc, hPen);
        HBRUSH hOldBrush = (HBRUSH)SelectObject(hdc, GetStockObject(NULL_BRUSH)); // Прозорий

        // ##### ВИПРАВЛЕНА ЛОГІКА ДЛЯ "ГУМОВОГО" СЛІДУ #####
        // x1, y1 - це центр
        // x2, y2 - це поточна позиція курсору
        LONG dx = abs(x2 - x1);
        LONG dy = abs(y2 - y1);
        Rectangle(hdc, x1 - dx, y1 - dy, x1 + dx, y1 + dy); // Малюємо симетрично
        // #######################################################
